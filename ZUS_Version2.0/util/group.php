<?php
	...
	class GroupDAO {
		...
		public static function get_group_list_large($pid, $category = 0, $limit = 24, $start = 0) {
			...
			$limit_plus = $limit+1;
			if ($category > 0) {
				$stmt->prepare('SELECT gid, logo, title, owner FROM community WHERE category=? AND privacy<'. Privacy::NonExist .' ORDER BY gid DESC LIMIT ?,?;');
				$stmt->bind_param('iii', $category, $start, $limit_plus);
			}
			else {
				$stmt->prepare('SELECT gid, logo, title, owner FROM community WHERE privacy<'. Privacy::NonExist .' ORDER BY gid DESC LIMIT ?,?;');
		...
		public static function get_group_logo($pid, $gid) {
			...
			$role = PeopleDAO::get_group_role_pid($pid, $gid);
			if ($role >= Role::Admin) {
				$group_logo['edit'] = 'window.location=\'logo.php?id='.$gid.'\'';
			}
			return $group_logo;
		}
		...
		public static function get_group_oper_button_list($pid, $gid) {
			...
			switch ($role) {
				...
				case Role::Admin:
					$main_oper = array(
									   'action' => 'window.location=\'logo.php?id='.$gid.'\'',
									   'class'  => 'edit_logo',
									   'title'  => '修改图标'
									   );
					array_push($oper_button['large'], $main_oper);
...
//获得群组成员并按性别分类
		...
		public static function get_group_event_list($pid, $gid, $limit = 6, $start = 0) {
			// #1 default value
			$event_list = array();
			...
			foreach ($event_id_list as $teid) {
				$event = EventDAO::get_event_basic_eid($teid);
				$event['action'] = array();
		...
		public static function get_info_list($pid, $gid) {
			...
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('SELECT title, ctime, description, category, size, tag, announcement FROM community WHERE gid=? LIMIT 1;');
			$stmt->bind_param('i', $gid);
			$stmt->execute();
			$result = $stmt->get_result();
			if ($row = $result->fetch_array(MYSQLI_ASSOC)) 
			{
				...
				if (!empty($row['tag'])) {
					$info_list['群组标签'] = strip_tags($row['tag']);
				}
