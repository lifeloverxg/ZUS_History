<?php
	...
	class EventDAO {
		...
		public static function get_event_list_large($pid, $category = 0, $limit = 24, $start = 0) {
			...
			$limit_plus = $limit+1;
			if ($category > 0) {
				$stmt->prepare('SELECT eid, logo, title, owner, gowner, issale FROM event WHERE category=? AND privacy<'. Privacy::NonExist .' ORDER BY eid DESC LIMIT ?,?;');
				$stmt->bind_param('iii', $category, $start, $limit_plus);
			}
			else {
				$stmt->prepare('SELECT eid, logo, title, owner, gowner, issale FROM event WHERE privacy<'. Privacy::NonExist .' ORDER BY eid DESC LIMIT ?,?;');
