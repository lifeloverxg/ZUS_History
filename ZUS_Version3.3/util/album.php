<?php
	...
	class AlbumDAO {
		...
		public static function add_photo_people($pid, $photo) {
			$aid = self::get_default_album_people($pid);
			if ($aid != 0) {
				$mysqli = MysqlInterface::get_connection();
				$stmt = $mysqli->stmt_init();
				$stmt->prepare('INSERT INTO photo (aid, owner, file) VALUES (?, ?, ?);');
				$stmt->bind_param('iis', $aid, $pid, $photo);
				$stmt->execute();
				$stmt->close();
			}
			return 0;
		...
		// set people album photo by id
		public static function add_photo_event($eid, $photo, $owner) {
			$aid = self::get_default_album_event($eid);
			if ($aid != 0) {
				$mysqli = MysqlInterface::get_connection();		
				$stmt = $mysqli->stmt_init();
				$stmt->prepare('INSERT INTO photo (aid, owner, file) VALUES (?, ?, ?);');
				$stmt->bind_param('iis', $aid, $owner, $photo);
				$stmt->execute();
				$stmt->close();
			}
			return 0;
