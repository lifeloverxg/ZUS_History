...
/*	搜索_New*/
	...
	$event_list_container = $PollDAO_obj->get_event_new_list($auth['uid']);
	$event_list_large = $event_list_container['event_list'];
	$next = $event_list_container['next'];
	$hot_event_list = $PollDAO_obj->get_hot_event_list($auth['uid']);
	$newest_event_list = $PollDAO_obj->get_newest_event_list($auth['uid']);
...
$bm->mark();
	...
	$create_event_option = $PollDAO_obj->create_event_option($auth['uid']);
...
//		echo $create_option;
		$eid = $PollDAO_obj->create_event_poll($auth['uid'], $event, $create_option);
		if ($eid > 0) {
			header('Location: ./poll_detail.php?eid='.$eid);
		}
		else {
...
//	include S_ROOT."event/spec/test.php";
	...
	{
		include S_ROOT."template/event/poll_frame.php";
	}
$bm->mark();
echo '<!-- '.$bm->report().'-->';
