<?php
	...
	class PollDAO {
		private $row;
		/*$eid,$title,$owner,$gowner,$start_time,$end_time,$repeat_type, $location, $logo, $description,$category,$size,$tag,$price,$vote_num,$verify,$latitude,$longitude,$geolable,$privacy,$ablum,$issale,$ctime ;*/
        ...
        public function get_event_data($eid){

        	$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('SELECT * FROM event_new WHERE eid=? LIMIT 1;');
		...
		public function get_event_logo($pid, $eid) {
			// #1 default value
			$event_logo = array(
			...
			// #3 check edit
			$role = self::get_poll_role_pid($pid, $eid);
			if ($role >= Role::Admin) {
		...
		/*++++++++++增加issale字段++++++++++*/
		public function get_event_issale($eid)
		{
			$issale = 0;
			if($this->row)
			{
		...
		public function get_event_oper_button_list($pid, $eid) 
		{
			...
			if ($this->get_event_privacy_eid($eid) == Privacy::NonExist) {
				$main_oper = array(
								   'action' => '',
								   'class'  => 'ghost_event',
								   'title'  => '不存在'
			...
			// #2 get large
			$role = self::get_poll_role_pid($pid, $eid);
			$issale = $this->get_event_issale($eid);
			switch ($role) {
				case Role::None:
...
//					array_push($oper_button['large'], $main_oper);
		...
		public function get_event_map_view($pid, $eid) {
			if($this->row)
			{
				$row=$this->row;
			}
			...
			// #2 fill values
			$role = self::get_poll_role_pid($pid, $eid);
			$privacy = $this->get_event_privacy_eid($eid);
			if ($role >= $privacy) {
		...
		public function get_event_member_list($pid, $eid, $limit = 12, $start = 0) {
			...
			foreach ($member_id_list as $tpid) 
			{
				...
				$admin['action'] = array();
				$trole = self::get_self_role_pid($tpid, $eid);
			...
			foreach ($member_id_list as $gender => $gender_id_list)
			{
				foreach ($gender_id_list as $tpid) 
				{
					...
					$member['action'] = array();
					$trole = self::get_poll_role_pid($tpid, $eid);
		...
		public function get_event_member_list_nogender($pid, $eid, $limit = 12, $start = 0) {
...
/*get all the members*/
		public function get_event_member_list_all($pid, $eid, $limit = 1000, $start = 0) {
			...
			// #2 fill title
			$basic = $this->get_event_basic_eid($eid);
	...
	//	$member_id_list = array_merge($member_id_list_gender['female'], $member_id_list_gender['male']);
		...
		public function get_event_recommend_list($pid, $eid, $limit = 6, $start = 0) {
		...
		public  function get_info_list($pid, $eid) 
		{   
			if($this->row)
			{
				$row=$this->row;
		...
		public function get_feed_list($pid, $eid, $tag_id = 0,$bd_start = 0, $bd_limit = 20, $cm_start = 0, $cm_limit = 3) {
			return BoardDAO::get_feed_list($pid, 'event', $eid, $tag_id, $bd_start, $bd_limit, $cm_start, $cm_limit);
		}

		// #ContentFunction
		// !!! [CF1] create event, return eid
		// public function create_event($pid, $event) 
		// {	
		...
		// 	self::set_poll_role_pid($pid, $eid, Role::Owner);
		// 	return $eid;
		// }

		//创建活动身份
		public function create_event_option($pid)
		{   
		...
		// !!! [CF2] join event
		public function join_event($pid, $eid) {
			...
			switch ($role) {
				case Role::None:
					if ($privacy >= Role::Member) {
						self::set_poll_role_pid($pid, $eid, Role::Pending);
					}
					else {
						self::set_poll_role_pid($pid, $eid, Role::Member);
					}
					return true;
				case Role::Invited:
					self::set_poll_role_pid($pid, $eid, Role::Member);
		...
		// !!! [CF3] leave event
		public function leave_event($pid, $eid) {
			$role = self::get_poll_role_pid($pid, $eid);
			switch ($role) {
				case Role::Member:
				case Role::Admin:
					self::set_poll_role_pid($pid, $eid, Role::None);
					return true;
			}
			return false;
		}
		...
		// !!! [CF4] delete event
		public function delete_event($pid, $eid) {
			$role = self::get_poll_role_pid($pid, $eid);
			switch ($role) {
				case Role::Owner:
					$this->set_event_privacy_eid($eid, Privacy::NonExist);
		...
		// !!! [CF5] upgrade admin
		public function upgrade_admin($pid, $tpid, $eid) {
			if ( self::get_poll_role_pid($pid, $eid) != Role::Owner) {
				return false;
			}
			$role = self::get_poll_role_pid($pid, $eid);
			switch ($role) {
				case Role::Member:
					self::set_poll_role_pid($tpid, $eid, Role::Admin);
					return true;
			}
			return false;
		}
		...
		// !!! [CF6] degrade admin
		public function degrade_admin($pid, $tpid, $eid) {
			if (self::get_poll_role_pid($pid, $eid) != Role::Owner) {
				return false;
			}
			$role = self::get_poll_role_pid($pid, $eid);
			switch ($role) {
				case Role::Admin:
					self::set_poll_role_pid($tpid, $eid, Role::Member);
					return true;
			}
			return false;
		}
		...
		// !!! [CF7] delete member
		public function delete_member($pid, $tpid, $eid) {
			if (self::get_poll_role_pid($pid, $eid) < Role::Admin) {
				return false;
			}
			$role = self::get_poll_role_pid($pid, $eid);
			switch ($role) {
				...
				case Role::Pending:
				case Role::Member:
					self::set_poll_role_pid($tpid, $eid, Role::None);
					return true;
			}
			return false;
		}
		...
		// !!! [CF8] edit event
		public function edit_info($pid, $eid, $event) {
			$role = self::get_poll_role_pid($pid, $eid);
			...
			// edit event
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('UPDATE event_new SET title=?, start_time=?, end_time=?, location=?, description=?, category=?, size=?, tag=?, price=?, privacy=? WHERE eid=?;');
		...
		public function get_event_basic_eid($eid) 
		{
			if($this->row)
			{
				$row=$this->row;
			...
			if ($row) 
			{
				$event['url'] = 'event/poll_detail.php?eid='.$eid;
				if (!empty($row['logo'])) 
				{
		...
		public function get_event_privacy_eid($eid) {
			// #1 default value
		...
		public function get_event_time_eid($eid) {
			$event_time = array(
								'start_time' => '',
								'end_time' => ''
			);
		 ...
		 */
		// set geocode
		public function set_event_geocode_eid($eid, $title, $latitude, $longitude) {
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('UPDATE event_new SET geolabel=?, latitude=?, longitude=? WHERE eid=?;');
			$role = Role::Member;
		...
		// set privacy
		public function set_event_privacy_eid($eid, $privacy) {
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('UPDATE event_new SET privacy=? WHERE eid=?;');
		...
		public function set_event_logo_eid($eid, $logo) {
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('UPDATE event_new SET logo=? WHERE eid=?;');
		...
		public function get_eid_list_people($pid) {
			$event_ids = array();
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
		...
		public function get_eid_list_group($gid) 
		{
			$event_ids = array();
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
		...
		public function get_event_basic_eid_large($eid) {
			// #1 default value
			$event = array(
						   'url' => '',
						   'image' => DefaultImage::Event. '_large.jpg',  
...
/*
		public static function get_index_event_list()
		{
			...
			{
				$index_event = PollDAO::get_event_basic_eid_large($ids);
...
*/
		public function get_index_event_list()
		{
			//============#1 请修改这个id_list===============
			$index_event_id_list = array(
										46,
			...
			foreach ($index_event_id_list as $ids)
			{
				$index_event = $this->get_event_basic_eid_large($ids);			
		...
		/*存储每一笔sale*/
		public function set_event_sale_eid_oid($pid, $eid, $oid)
		{
			$mysqli = MysqlInterface::get_connection();
		...
		/*+++++++++++++++热门活动+++++++++++++++*/
		public function get_hot_event_list($pid, $start = 0, $limit = 5)
		{
			...
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('SELECT eid, logo, title, privacy FROM event_new WHERE privacy<'. Privacy::NonExist .' ORDER BY ctime DESC LIMIT ?,?;');
			...
			usort($hot_event_list_unsorted, "PollDAO::hot_cmp");
		...
		/*+++++++++++++++最新活动+++++++++++++++*/
		public function get_newest_event_list($pid, $start = 0, $limit = 5)
		{
			...
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('SELECT eid, logo, title, privacy, ctime FROM event_new WHERE privacy<'. Privacy::NonExist .' ORDER BY ctime DESC LIMIT ?,?;');
			$stmt->bind_param('ii', $start, $limit_plus);
			$stmt->execute();
			$result = $stmt->get_result();
			while ($row = $result->fetch_array(MYSQLI_ASSOC)) 
		...
		/*===============dream event poll===============*/
		public function create_event_poll($pid, $event, $gid = 0) 
		{
			// check permission
			if ($pid <= 0) {
				return 0;
			...
			// insert event
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('INSERT INTO event_new (title, owner, gowner, start_time, end_time, location, logo, description, category, size, tag, price, ctime, privacy, verify) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);');
			$stmt->bind_param('siisssssiisssii', $event['title'], $pid, $gid, $event['start_time'], $event['end_time'], $event['location'], $event['logo'], $event['description'], $event['category'], $event['size'], $event['tag'], $event['price'], date('Y-m-d H:i:s'), $event['privacy'], $event['verify']);
			$stmt->execute();
		...
		public function get_poll_role_pid($pid, $eid) {
			$role = Role::None;
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('SELECT * FROM people2poll WHERE pid=? AND eid=? LIMIT 1;');
		...
		public function vote_poll($pid, $eid) {
		...
		// public function set_group_role_pid($pid, $gid, $role) {
		// 	$mysqli = MysqlInterface::get_connection();
		// 	$stmt = $mysqli->stmt_init();
		...
		// set poll role with pid
		public function set_poll_role_pid($pid, $eid, $role) {
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('INSERT INTO people2poll (pid, eid, role) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE role=?;');
		...
		public function get_event_new_list($pid, $category = 0, $limit = 24, $start = 0) {
			// #1 default value
			...
			$limit_plus = $limit+1;
			if ($category > 0) {
				$stmt->prepare('SELECT * FROM event_new WHERE category=? AND privacy<'. Privacy::NonExist .' ORDER BY vote_num DESC LIMIT ?,?;');
				$stmt->bind_param('iii', $category, $start, $limit_plus);
			}
			else {
				$stmt->prepare('SELECT * FROM event_new WHERE privacy<'. Privacy::NonExist .' ORDER BY vote_num DESC LIMIT ?,?;');
				$stmt->bind_param('ii', $start, $limit_plus);
			}
			$stmt->execute();
			$result = $stmt->get_result();
			...
			while ($row = $result->fetch_array(MYSQLI_ASSOC)) 
			{
				...
				// #2.1 basic fields
				$event['info'] = $this->get_info_list($pid, $eid);
				$event['url']   = 'event/poll_detail.php?eid='.$eid;
				if (!empty($row['logo'])) {
					$event['image'] = $row['logo'].'_large.jpg';
				}
				...
				//find how many people vote for the event
				$event['vote_num'] = $row['vote_num'];
