<?php
	...
	class EventDAO {
		...
		public static function get_event_list_large($pid, $category = 0, $limit = 24, $start = 0) {
			...
			$limit_plus = $limit+1;
			if ($category > 0) {
				$stmt->prepare('SELECT eid, logo, title, owner, gowner FROM event WHERE category=? AND privacy<'. Privacy::NonExist .' ORDER BY eid DESC LIMIT ?,?;');
				$stmt->bind_param('iii', $category, $start, $limit_plus);
			}
			else {
				$stmt->prepare('SELECT eid, logo, title, owner, gowner FROM event WHERE privacy<'. Privacy::NonExist .' ORDER BY eid DESC LIMIT ?,?;');
...
/*get all the members*/
		...
		public static function get_info_list($pid, $eid) {
			...
			if ($row = $result->fetch_array(MYSQLI_ASSOC)) {
				...
				if (!empty($row['location'])) {
					$info_list['活动地点'] = strip_tags($row['location']);
				}
				...
				if (!empty($row['tag'])) {
					$info_list['活动标签'] = strip_tags($row['tag']);
				}
				if (!empty($row['price'])) {
		...
		public static function create_event($pid, $event, $gid = 0) {
			// check permission
		...
		public static function edit_info($pid, $eid, $event) {
			...
			$stmt->bind_param('sssssiisdi', $event['title'], $event['start_time'], $event['end_time'], $event['location'], $event['description'], $event['category'], $event['size'], $event['tag'], $event['price'], $eid);
			$stmt->execute();
			return true;
		}
