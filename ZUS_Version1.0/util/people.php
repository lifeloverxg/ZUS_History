<?php
	...
	class PeopleDAO {
		...
		public static function get_people_logo($pid, $tpid) {
			...
			// #3 check edit
			if ($pid == $tpid) {
				$people_logo['edit'] = 'window.location=\'logo.php?id='.$tpid.'\'';
			}
			return $people_logo;
		}
		...
		public static function get_people_oper_button_list($pid, $tpid) {
			...
			switch ($relation) {
				...
				case Relation::Self:
					$main_oper = array(
									   'action' => 'window.location=\'logo.php?id='.$pid.'\'',
									   'class'  => 'edit_logo',
									   'title'  => '修改头像'
									   );
					array_push($oper_button['large'], $main_oper);
		...
		public static function get_info_list($pid, $tpid) {
			...
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('SELECT name, signature, birth, gender, education, hometown, marriage, phone, email, address, hobby, privacy FROM people WHERE pid=? LIMIT 1;');
			$stmt->bind_param('i', $tpid);
			$stmt->execute();
			$result = $stmt->get_result();
			if ($row = $result->fetch_array(MYSQLI_ASSOC)) {
		...
		public static function edit_info($pid, $people) {
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('UPDATE people SET name=?, signature=?, birth=?, gender=?, education=?, hometown=?, marriage=?, phone=?, email=?, address=?, hobby=?, privacy=? WHERE pid=?');
			$stmt->bind_param('sssisssssssii', $people['name'], $people['signature'], $people['birth'], $people['gender'], $people['education'], $people['hometown'], $people['marriage'], $people['phone'], $people['email'], $people['address'], $people['hobby'], $people['privacy'], $pid);
			$stmt->execute();
			return true;
		}
		...
		// get event member id list with role
		...
		// get id list classified by relations(0: following, 1： follower, 2: recommend)
