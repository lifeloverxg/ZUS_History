<?php
	...
	class ArticleDAO 
	{
		...
		public static function post_article($pid, $gid, $article) 
		{
			...
			$mysqli = MysqlInterface::get_connection();
			$stmt = $mysqli->stmt_init();
			$stmt->prepare('INSERT INTO article (bid, title, content, category, tag) VALUES (?, ?, ?, ?, ?);');
			$stmt->bind_param('issis', $bid, $article['title'], $article['content'], $article['category'], $article['tag']);
			$stmt->execute();
			$stmt->close();
...
*/
		...
		public static function get_my_article_pid($pid)
		{
			...
			$article_id_list = ArticleDAO::get_arid_list_pid($pid);

			foreach ($article_id_list as $key => $article_id)
			{
				$article_temp['author'] = PeopleDAO::get_people_basic_pid($article_id['author_id']);
				$article_temp['detail'] = ArticleDAO::get_article_basic_arid($article_id['article_id']);
		...
		public static function get_all_article_arids()
		{
			...
			$article_id_list = ArticleDAO::get_arid_list_all();
		...
		public static function get_friend_article_list_pid($pid , $limit = 6, $start = 0)
		{
			...
			{
				$article_temp = ArticleDAO::get_my_article_pid($friend_id);
		...
		public static function get_article_list_small($pid)
		{
			...
			$my_article = ArticleDAO::get_my_article_pid($pid);
			$friend_article = ArticleDAO::get_friend_article_list_pid($pid);
		...
		public static function get_article_list_large_all($limit = 10, $start = 0)
		{
			...
			$all_article = ArticleDAO::get_all_article_arids();
		...
		{
			$article = ArticleDAO::get_article_basic_arid($arid);
